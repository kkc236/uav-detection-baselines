"""Launch the frozen VisDrone LRS-GFDR arm under Formal100 seed 0."""
...
def build_settings(data_yaml: Path, output_root: Path, name: str) -> dict[str, Any]:
    return {
        **FROZEN_SETTINGS,
        "model": str(LRS_GFDR_CONFIG.resolve()),
        "save_period": -1,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--name", default="formal-seed0-lrs_gfdr-v1")
    ...
    record = {
        "format_version": 1,
        "method_revision": "lrs-gfdr-v1",
        "method": "lrs_gfdr",
        "source": current_source_identity(),
        "config": {"path": str(LRS_GFDR_CONFIG.resolve()), "sha256": _sha256(LRS_GFDR_CONFIG)},
