...
mkdir -p "$STORAGE_ROOT/datasets" "$STORAGE_ROOT/runs/ioqc-sa" "$STORAGE_ROOT/logs" \
  "$STORAGE_ROOT/secrets" "$STORAGE_ROOT/results-checkout"
...
print("Adaptive batch policy:", policy)
if "5090" in profile.name:
    assert "sm_120" in __import__("torch").cuda.get_arch_list(), "Installed PyTorch wheel lacks RTX 5090 support"
PY
...
printf '\nSetup complete. Write a new fine-grained GitHub token to:\n  %s\n' "$TOKEN_FILE"
