...
def variant_identity(variant: str) -> VariantIdentity:
    if variant == "baseline":
        return VariantIdentity(
            run_name="scratch-rtdetr-l-vsf-matched-baseline-100ep",
            tag="vsf-rmr-matched-baseline-live",
            asset_prefix="vsf-matched-baseline-last",
            release_name="VSF-RMR Matched RT-DETR-L Baseline Checkpoints",
        )
    if variant == "vsf-rmr":
        return VariantIdentity(
            run_name="scratch-rtdetr-l-vsf-rmr-100ep",
            tag="vsf-rmr-rtdetr-l-live",
            asset_prefix="vsf-rmr-last",
            release_name="VSF-RMR RT-DETR-L Live Checkpoints",
        )
...
def classify_child_exit(returncode: int, output: str) -> str:
    ...
    if "nonfinite_loss" in normalized or "nonfinite_vsf_rmr" in normalized:
...
def build_child_command(
    *,
    ...
    command = [
        python_executable,
        str(ROOT / "scripts" / "train_rtdetr_vsf_rmr.py"),
        "--variant",
        variant,
        "--epochs",
        str(epochs),
...
def acquire_pid_lock(path: Path) -> None:
    ...
    if path.exists():
        try:
            ...
            owner = -1
        if owner > 0 and _pid_is_alive(owner):
            raise RuntimeError(f"VSF-RMR supervisor is already running with PID {owner}")
...
def _status(state: AdaptiveTrainingState, *, variant: str, **extra: object) -> dict[str, object]:
    return {
...
def run_supervisor(args: argparse.Namespace) -> int:
    devices = parse_device_indices(args.device)
    profile = detect_gpu_profile(devices[0])
    identity = variant_identity(args.variant)
    run_name = args.name or identity.run_name
    state_path = args.state.resolve()
    run_dir = (args.project / run_name).resolve()
    ...
    try:
        while state.completed_epoch < args.epochs:
            if shutil.disk_usage(args.project.resolve()).free / 1024**3 < args.min_free_gib:
                state.last_event = "low_disk_stop"
                save_adaptive_state(state_path, state)
                _atomic_json(args.status, _status(state, variant=args.variant, process_state="low_disk_stop"))
                return 3
            ...
            with args.log.open("a", encoding="utf-8") as log_file:
                log_file.write(
                    f"\nSUPERVISOR variant={args.variant} batch={state.current_batch} "
                    f"amp={state.amp_enabled} epoch={state.completed_epoch} resume={resume}\n"
                )
                log_file.flush()
            ...
            if received_signal is not None:
                ...
                _atomic_json(
                    args.status,
                    _status(state, variant=args.variant, process_state="signal_stop", signal=received_signal),
                )
            ...
            _atomic_json(
                args.status,
                _status(state, variant=args.variant, process_state=result, returncode=child.returncode),
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="GPU-adaptive matched baseline and VSF-RMR supervisor.")
