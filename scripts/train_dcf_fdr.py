"""Launch one source-bound DCF-FDR Formal100 seed-0 experiment."""
...
DCF_FDR_CONFIG = ROOT / "configs" / "rtdetr-l-dcf-fdr.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train DCF-FDR under Formal100 seed0.")
    parser.add_argument("--dataset-root", type=Path, required=True)
    parser.add_argument("--initial-state", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, required=True)
    parser.add_argument("--name")
    parser.add_argument("--arm", choices=("clean", "dcf"), default="dcf")
    parser.add_argument("--dry-run", action="store_true")
    return parser


def build_settings(
    *, data_yaml: Path, output_root: Path, name: str | None = None, arm: str = "dcf"
) -> dict[str, Any]:
    if arm not in {"clean", "dcf"}:
        raise ValueError(f"unknown DCF ablation arm: {arm}")
    config = CLEAN_FDR_CONFIG if arm == "clean" else DCF_FDR_CONFIG
    return {
        **FROZEN_SETTINGS,
        ...
        "name": name or f"formal-seed0-{arm}-fdr-v1",
        "exist_ok": False,
    }
...
def build_launch_record(
    *,
    ...
    config_path = Path(str(settings["model"])).resolve()
    initial_state_path = Path(initial_state_path).resolve()
    return {
        "format_version": 1,
        "method": "dcf_fdr" if config_path == DCF_FDR_CONFIG.resolve() else "clean_fdr",
        "source": dict(source_identity),
        "config": {"path": str(config_path), "sha256": _file_sha256(config_path)},
        "initial_state": {
...
def main(argv: list[str] | None = None) -> int:
    ...
    settings = build_settings(
        data_yaml=data_yaml, output_root=output_root, name=args.name, arm=args.arm
    )
