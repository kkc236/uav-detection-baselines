"""Launch the conflict-safe LRS-GFDR/Capacity-BPDD/FIA Formal100 arm."""
...
from src.rtdetr_bpdd_capacity_fia import (  # noqa: E402
    ...
    CapacityBPDDFIASafeTrainer,
)


METHOD = "lrs_gfdr_capacity_v3_safe_bpdd_fia"
METHOD_REVISION = "v3-safe-loc-kd-decay80-100-no-cls-kd"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train Formal100 conflict-safe Capacity-BPDD + FIA"
    )
    parser.add_argument("--dataset-root", type=Path, required=True)
    parser.add_argument("--initial-state", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, required=True)
    parser.add_argument("--name", default="formal-seed0-lrs_gfdr_capacity_v3_safe_bpdd_fia")
...
def build_settings(data_yaml: Path, output_root: Path, name: str) -> dict[str, Any]:
    return {
        **FROZEN_SETTINGS,
        "model": str(CAPACITY_BPDD_FIA_SAFE_CFG.resolve()),
...
def main(argv: list[str] | None = None) -> int:
    ...
    record = {
        ...
        "config": {
            "path": str(CAPACITY_BPDD_FIA_SAFE_CFG.resolve()),
            "sha256": _sha256(CAPACITY_BPDD_FIA_SAFE_CFG),
        },
        "initial_state": {
            "path": str(initial_state.resolve()),
            "sha256": _sha256(initial_state.resolve()),
        },
    ...
    trainer = CapacityBPDDFIASafeTrainer(
        overrides=settings,
