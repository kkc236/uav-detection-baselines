...
COMBINED = ROOT / "configs" / "rtdetr-l-fdr-bpdd-fia.yaml"
...
def test_combined_yaml_inserts_one_fia_only_on_decoder_p3() -> None:
    combined = _load(COMBINED)
    head = combined["head"]

    assert len(head) == 20
    assert sum(row[2] == "FIA" for row in head) == 1
    assert head[12] == [21, 1, "FIA", [256]]

    # Absolute model indices 23--28 rebuild stock P4/P5.  P4 must explicitly
    # read the unmodified P3 at index 21, never the new FIA output at index 22.
    assert head[13][0] == 21
