...
from src.fia import FIA, FIAAttention, FIABaseBlock
...
def test_fia_starts_as_bit_exact_identity_for_256_channels() -> None:
    module = FIA(256)
...
def test_fia_contains_two_depthwise_dual_residual_blocks_and_joint_attention() -> None:
    module = FIA(256)

    assert len(module.refine) == 3
    assert isinstance(module.refine[0], FIABaseBlock)
    assert isinstance(module.refine[1], FIABaseBlock)
    assert isinstance(module.refine[2], FIAAttention)
...
def test_fia_rejects_invalid_channel_configuration(channels: object) -> None:
    with pytest.raises((TypeError, ValueError), match="channels"):
        FIA(channels)  # type: ignore[arg-type]


def test_fia_rejects_non_bchw_and_wrong_channel_inputs() -> None:
    module = FIA(256)
...
def test_fia_private_path_reaches_every_parameter_when_gate_is_open() -> None:
...
def test_fia_construction_is_deterministic_and_fork_rng_isolated() -> None:
    torch.manual_seed(71)
    global_state = torch.random.get_rng_state().clone()

    with torch.random.fork_rng():
        torch.manual_seed(20000)
        first = _state_dict(FIA(256))
    torch.testing.assert_close(torch.random.get_rng_state(), global_state, rtol=0, atol=0)

    with torch.random.fork_rng():
        torch.manual_seed(20000)
        second = _state_dict(FIA(256))
...
def test_fia_state_dict_round_trip_preserves_exact_output() -> None:
    with torch.random.fork_rng():
...
def test_fia_preserves_cpu_dtype_and_device(dtype: torch.dtype) -> None:
    module = FIA(256).to(dtype=dtype, device="cpu")
...
def test_fia_preserves_cuda_dtype_and_device() -> None:
    module = FIA(256).to(dtype=torch.float16, device="cuda")
