...
Standalone VSF-RMR routes the three 256-channel Hybrid Encoder features immediately before the RT-DETR decoder. It predicts an ordered scale field, mixes only adjacent pyramid levels, and applies zero-initialized residual corrections. The matched baseline and VSF-RMR use the same scratch protocol with `mosaic=0`, `mixup=0`, `scale=0.5`, `perspective=0`, seed 0, 640-pixel input, and 100 epochs:

- Local matched baseline: `C:\uav_env\Scripts\python.exe scripts\train_rtdetr_vsf_rmr.py --variant baseline --batch 1`
- Local standalone VSF-RMR: `C:\uav_env\Scripts\python.exe scripts\train_rtdetr_vsf_rmr.py --variant vsf-rmr --batch 1`
- Cost benchmark: `C:\uav_env\Scripts\python.exe scripts\benchmark_vsf_rmr.py --batches 1 8`

The VSF-RMR acceptance limits are at most 5% inference-time increase, 3% parameter increase, 5% GFLOPs increase, and approximately 10% training-time increase. BTD-SE, IOQC-SA, and VSF-RMR have separate model entry points, losses, run directories, GitHub Release tags, and checkpoint prefixes; standalone ablations do not combine innovations.
