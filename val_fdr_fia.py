...
# train_fdr_fia.py 使用 FDRRTDETRDetectionModel 训练。
# 加载 best.pt 前必须导入自定义模型类，并注册 FIA / FDRRTDETRDecoder。
from src.rtdetr_fdr import FDRRTDETRDetectionModel, register_fdr_module  # noqa: F401
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=ROOT
        / "runs"
        / "fdr-fia-seed0-20260815-211203"
        / "weights"
        / "best.pt",
    )
    args = parser.parse_args()

    model_path = str(args.weights.expanduser().resolve())
    ...
    if model.task == "detect":
        ...
        with open(
            result.save_dir / "paper_data.txt",
            "w+",
            errors="ignore",
            encoding="utf-8",
        ) as f:
