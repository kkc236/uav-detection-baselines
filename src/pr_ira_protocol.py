"""Immutable Screen30 and Formal100 authority for PR-IRA experiments."""
...
_FORMAL_VARIANTS = ("fdr_bpdd_pr_ira",)
...
PR_IRA_PROTOCOL: dict[str, Any] = {
    "design": "ultralytics-rtdetr-l-fdr-bpdd-pr-ira-v1",
    "fdr_authority": {
...
def build_run_identity(
    ...
    *,
    ...
    if stage not in PR_IRA_PROTOCOL["stages"]:
        raise ValueError(f"unknown PR-IRA stage: {stage}")
    if variant not in PR_IRA_PROTOCOL["variants"]:
        raise ValueError(f"unknown PR-IRA variant: {variant}")
    eligible = PR_IRA_PROTOCOL["stages"][stage]["eligible_variants"]
    if variant not in eligible:
        raise ValueError(f"PR-IRA variant {variant!r} is not eligible for {stage}")
    if type(seed) is not int or seed != PR_IRA_PROTOCOL["seed"]:
        raise ValueError("PR-IRA protocol is frozen to seed0")
    ...
    run_id = (
        ...
        f"{PR_IRA_PROTOCOL_SHA256[:12].lower()}"
    )
    return {
        "source_sha256": source_sha256,
        "protocol_sha256": PR_IRA_PROTOCOL_SHA256,
        "fdr_protocol_sha256": FDR_PROTOCOL_SHA256,
        "initial_state_sha256": FDR_INITIAL_STATE_SHA256,
        "dataset_sha256": PR_IRA_PROTOCOL["dataset"]["sha256"],
        "screen_subset_sha256": PR_IRA_PROTOCOL["dataset"]["screen_sha256"],
        "run_id": run_id,
        "stage": stage,
        "variant": variant,
        "seed": seed,
